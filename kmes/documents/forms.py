...
# 	"""Form for bulk uploading multiple documents (simplified version)."""
#
# 	project = forms.ModelChoiceField(
# 			queryset=None,
# 			widget=forms.Select(attrs={'class': 'form-select'})
...
# 			help_text="Upload a ZIP file containing multiple documents"
# 			)
# 	file1 = forms.FileField(
# 			required=False,
# 			widget=forms.FileInput(attrs={'class': 'form-control'}),
# 			help_text="Individual file upload"
# 			)
...
# 		files = [cleaned_data.get(f'file{i}') for i in range(1, 6) if cleaned_data.get(f'file{i}')]
#
# 		if not zip_file and not files:
# 			raise forms.ValidationError("Please upload a ZIP file or at least one individual file.")
#
# 		return cleaned_data
# #
# # class DocumentBulkUploadForm(forms.Form):
